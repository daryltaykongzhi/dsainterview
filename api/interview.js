import { INTERVIEW_SYSTEM_PROMPT, getRealExamples } from './prompts.js';
...
export default async function handler(req, res) {
    ...
    try {
        ...
        // 3. Get real past questions for this talent area
        const realExamples = getRealExamples(talentArea);
        
        // 4. Prepare the prompt with user inputs AND real examples
        const formattedPrompt = INTERVIEW_SYSTEM_PROMPT
            .replace(/{{course}}/g, talentArea)
            .replace(/{{poly}}/g, jc)
            .replace(/{{real_examples}}/g, realExamples);
        
        // Optional: Log for debugging
        console.log(`Generating DSA questions for: ${talentArea} at ${jc}`);
